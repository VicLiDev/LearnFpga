...
module auto_tb;
    reg clk = 0, rst_n = 0;
    reg [7:0] a, b;
    wire [10:0] result_np, result_pl;
    always #5 clk = ~clk;
    adder_no_pipeline u_np (.clk(clk),.a(a),.b(b),.result(result_np));
    adder_pipeline_3stage u_pl (.clk(clk),.rst_n(rst_n),.a(a),.b(b),.result(result_pl));
    initial begin
        \$dumpfile("${VCD_FILE}");
        \$dumpvars(0, auto_tb);
        a = 10; b = 20; #10; rst_n = 1;
        #100;
        a = 50; b = 75; #100;
        a = 100; b = 150; #100;
        \$display("Non-pipeline result: %0d", result_np);
        \$display("Pipeline result:     %0d", result_pl);
...
sim() {
    ...
    echo "========================================"
    auto_tb
    iverilog -o "${OUT_DIR}/sim_wave" "${OUT_DIR}/auto_tb.v" "${PRJ_DIR}/01_pipeline_adder.v" 2>&1
