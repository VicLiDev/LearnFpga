...
module auto_tb;
    reg clk = 0, rst_n = 0;
    reg signed [7:0] xin = 16'sd100;
    wire signed [15:0] yout;
    always #5 clk = ~clk;
    fir_filter #(.COEFF_WIDTH(16),.DATA_WIDTH(8),.ACC_WIDTH(32)) u_dut (
        .clk(clk),.rst_n(rst_n),.xin(xin),.yout(yout)
    );
    integer i;
    initial begin
        \$dumpfile("${VCD_FILE}");
        ...
        for (i = 0; i < 20; i = i + 1) begin
            xin = \$random; #10;
            \$display("cycle=%0d, xin=%0d, yout=%0d", i, xin, yout);
        end
...
sim() {
    ...
    echo "========================================"
    auto_tb
    iverilog -o "${OUT_DIR}/sim_wave" "${OUT_DIR}/auto_tb.v" "${PRJ_DIR}/01_fir_filter.v" 2>&1
